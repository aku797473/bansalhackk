  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 selection:bg-emerald-500/30 selection:text-emerald-600 font-sans overflow-x-hidden transition-colors duration-300">
      ...
      <nav className="fixed top-0 inset-x-0 z-[100] px-2 sm:px-4 pt-4">
        <div className="max-w-7xl mx-auto h-16 sm:h-20 border border-slate-100 dark:border-white/5 bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl rounded-2xl sm:rounded-[1.5rem] px-4 sm:px-8 flex items-center justify-between shadow-2xl">
      ...
      {/* Hero Section - Fixed Header Cover Issue */}
      <header className="relative min-h-screen flex flex-col lg:flex-row overflow-hidden">
         
         {/* Side 1: Content (Added pt-40 to prevent navbar cover) */}
         <div className="w-full lg:w-1/2 min-h-[70vh] lg:min-h-screen flex items-center px-6 sm:px-12 lg:px-24 pt-48 sm:pt-56 lg:pt-32 relative z-20 bg-white dark:bg-slate-950">
            <div className="max-w-xl w-full">
               ...
               <h1 className="text-5xl sm:text-7xl lg:text-[9rem] font-black leading-[0.85] lg:leading-[0.8] tracking-tighter mb-10 sm:mb-14 uppercase">
         ...
         <div className="w-full lg:w-1/2 h-[50vh] sm:h-[60vh] lg:h-screen relative overflow-hidden bg-slate-50 dark:bg-slate-900/10 border-t lg:border-t-0 lg:border-l border-slate-100 dark:border-slate-800">
            <div className="absolute inset-0 z-10">
               <ThreeHero key="landing-hero-3d-v2" />
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 sm:gap-16">
               {[
                 ...
                 <div key={stat.label} className="text-center lg:text-left group">
                    <div className="text-5xl sm:text-8xl font-black tracking-tighter mb-2 sm:mb-4 group-hover:text-emerald-600 transition-colors">{stat.value}</div>
      ...
      {/* Feature Grid */}
      <section className="features-section py-24 sm:py-40 px-4 sm:px-8">
         <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
               {['weather', 'crop', 'market'].map((key) => (
                 <div key={key} className="p-12 sm:p-20 bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-[3.5rem] sm:rounded-[5rem] hover:border-emerald-600 transition-all duration-500 cursor-pointer group relative overflow-hidden shadow-sm hover:shadow-2xl">
                    <div className="text-[11px] sm:text-[13px] font-black uppercase tracking-[0.4em] mb-12 sm:mb-20 opacity-40 group-hover:opacity-100 group-hover:text-emerald-600">0{key === 'weather' ? 1 : key === 'crop' ? 2 : 3}</div>
                    <h3 className="text-4xl sm:text-6xl font-black tracking-tighter uppercase mb-6 sm:mb-12 leading-none">{t(`landing.modules.${key}.label`)}</h3>
                    ...
                    <div className="mt-12 sm:mt-24 w-16 h-16 sm:w-20 sm:h-20 rounded-2xl sm:rounded-[2.5rem] bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-2xl font-black group-hover:bg-emerald-600 group-hover:text-white transition-all">
                       →
                    </div>
                 </div>
               ))}
      ...
      {/* Footer */}
      <footer className="py-24 px-8 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950">
         <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-12">
            <div className="flex flex-col items-center md:items-start gap-6">
               ...
               <p className="text-[10px] sm:text-[11px] font-black uppercase tracking-[0.4em] text-slate-400">© 2026 AGRI-INTELLIGENCE | LOCAL JWT SECURED</p>
            </div>
